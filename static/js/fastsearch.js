...
function executeSearch(term) {
  ...
  if (results.length === 0) { // no results based on what was typed into the input box
    ...
    //   console.log('item: %d, title: %s', item, results[item].item.title)
      ...
      searchitems = searchitems + '<div class="pl-1 pr-1 pl-sm-2 pr-sm-2 pl-lg-4 pr-lg-4 pl-xl-0 pr-xl-0">' + '<a href="' + results[item].item.permalink + '">' + results[item].item.title + '</a>' + '<div class="post-meta d-flex flex-column flex-sm-row text-muted mt-1 mb-1"> <div class="mr-sm-4">' + '<i class="far fa-folder fa-fw"></i>' + results[item].item.categories + '</div> <div><i class="fa fa-tag fa-fw"></i>'+ results[item].item.tags +'</div></div>' + '<p>' + results[item].item.contents + '</p></div>'
